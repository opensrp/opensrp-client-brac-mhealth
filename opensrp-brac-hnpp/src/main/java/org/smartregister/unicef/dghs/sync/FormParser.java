...
import static org.smartregister.unicef.dghs.utils.HnppConstants.EVENT_TYPE.ANC1_REGISTRATION;
import static org.smartregister.unicef.dghs.utils.HnppConstants.EVENT_TYPE.ANC1_REGISTRATION_OOC;
import static org.smartregister.unicef.dghs.utils.HnppConstants.EVENT_TYPE.ANC2_REGISTRATION;
import static org.smartregister.unicef.dghs.utils.HnppConstants.EVENT_TYPE.ANC2_REGISTRATION_OOC;
import static org.smartregister.unicef.dghs.utils.HnppConstants.EVENT_TYPE.ANC3_REGISTRATION;
import static org.smartregister.unicef.dghs.utils.HnppConstants.EVENT_TYPE.ANC3_REGISTRATION_OOC;
...
import static org.smartregister.unicef.dghs.utils.HnppConstants.EVENT_TYPE.EYE_TEST;
...
import static org.smartregister.unicef.dghs.utils.HnppConstants.EVENT_TYPE.PNC_REGISTRATION_AFTER_48_hour;
import static org.smartregister.unicef.dghs.utils.HnppConstants.EVENT_TYPE.PNC_REGISTRATION_AFTER_48_hour_OOC;
import static org.smartregister.unicef.dghs.utils.HnppConstants.EVENT_TYPE.PNC_REGISTRATION_BEFORE_48_hour;
import static org.smartregister.unicef.dghs.utils.HnppConstants.EVENT_TYPE.PNC_REGISTRATION_BEFORE_48_hour_OOC;
import static org.smartregister.unicef.dghs.utils.HnppConstants.EVENT_TYPE.PREGNANCY_OUTCOME;
import static org.smartregister.unicef.dghs.utils.HnppConstants.EVENT_TYPE.PREGNANCY_OUTCOME_OOC;
import static org.smartregister.unicef.dghs.utils.HnppConstants.EVENT_TYPE.REFERREL_FOLLOWUP;
import static org.smartregister.unicef.dghs.utils.HnppConstants.EVENT_TYPE.SS_INFO;
import static org.smartregister.unicef.dghs.utils.HnppConstants.EVENT_TYPE.WOMEN_PACKAGE;
import static org.smartregister.unicef.dghs.utils.HnppConstants.EVENT_TYPE.WOMEN_REFERRAL;
...
public class FormParser {
            ...
            if (!StringUtils.isEmpty(eventJson)) {
                try {
                    ...
                    for (String encounter_type : encounter_types) {
                        ...
                        String ssName = HnppDBUtils.getBlockName(base_entity_id);
                        log.setBlockName(ssName);
                        ...
                        if(HOME_VISIT_FAMILY.equalsIgnoreCase(encounter_type)){
                            ...
                            ssName = HnppDBUtils.getBlockNameFromFamilyTable(base_entity_id);
                            log.setBlockName(ssName);
                        }else{
                        ...
                        if(isInserted!=-1){
                            Log.d(FormParser.class.getSimpleName(), "Encounter type: "+ encounter_type);
     ...
     */
    private static void processInstitutionalDeliveryForTarget(){
...
//    }
    ...
    private static boolean isNeedToAddStockTable(String eventType,HashMap<String, String> details){
        String targetName = StockRepository.getTargetName(eventType);
        if(TextUtils.isEmpty(targetName)) return false;
        if(details.containsKey("add_to_stock")&&!StringUtils.isEmpty(details.get("add_to_stock"))) {
            String value = details.get("add_to_stock");
    ...
    private static void processHHVisitForm(HashMap<String, String> details, VisitLog log) {
        try{
    ...
    private static void processSimprintsVerification(VisitLog log, HashMap<String, String> details) {
        if(details.containsKey("is_verified")&&!StringUtils.isEmpty(details.get("is_verified"))) {
            ...
            if(!TextUtils.isEmpty(value) && value.equalsIgnoreCase("true")){
                HnppApplication.getIndicatorRepository().updateValue("is_verified",value,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",log.getBlockName(),log.getBaseEntityId());
        ...
        if(details.containsKey("is_identified")&&!StringUtils.isEmpty(details.get("is_identified"))) {
            ...
            if(!TextUtils.isEmpty(value) && value.equalsIgnoreCase("true")){
                HnppApplication.getIndicatorRepository().updateValue("is_identified",value,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",log.getBlockName(),log.getBaseEntityId());
    ...
    private static void processIndicator(String baseEntityId,String encounter_type, VisitLog log, HashMap<String,String>details,String formSubmissionId){
        LocalDate localDate = new LocalDate(log.getVisitDate());
        switch (encounter_type){
            ...
            case ELCO:
                if(details.containsKey("familyplanning_method_known")&&!StringUtils.isEmpty(details.get("familyplanning_method_known"))) {
                    ...
                    HnppApplication.getIndicatorRepository().updateValue("familyplanning_method_known",value,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",log.getBlockName(),log.getBaseEntityId());
                ...
                if(details.containsKey("familyplanning_method")&&!StringUtils.isEmpty(details.get("familyplanning_method"))) {
                    ...
                    HnppApplication.getIndicatorRepository().updateValue("familyplanning_method",value,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",log.getBlockName(),log.getBaseEntityId());
                ...
                if(details.containsKey("distribution_location")&&!StringUtils.isEmpty(details.get("distribution_location"))) {
                    ...
                    HnppApplication.getIndicatorRepository().updateValue("distribution_location",value,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",log.getBlockName(),log.getBaseEntityId());
            ...
            case ANC3_REGISTRATION:
                if(details.containsKey("other_source_anc_1")&&!StringUtils.isEmpty(details.get("other_source_anc_1"))) {
                    ...
                    if(value!=null && !value.equalsIgnoreCase("brac")){
                        HnppApplication.getIndicatorRepository().updateValue(HnppConstants.INDICATOR.ANC_OTHER_SOURCE,"true",localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",log.getBlockName(),log.getBaseEntityId());
                    }
                ...
                if(details.containsKey("vaccination_tt_dose_completed")&&!StringUtils.isEmpty(details.get("vaccination_tt_dose_completed"))) {
                    ...
                    HnppApplication.getIndicatorRepository().updateValue(HnppConstants.INDICATOR.ANC_TT,value,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",log.getBlockName(),log.getBaseEntityId());
                    ...
                    if(!TextUtils.isEmpty(value)){
                        if(value.equalsIgnoreCase("tt1") || value.equalsIgnoreCase("tt2") || value.equalsIgnoreCase("tt3")
                         || value.equalsIgnoreCase("tt4")){
                            HnppApplication.getIndicatorRepository().updateValue(HnppConstants.INDICATOR.ANC_TT,"yes",localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",log.getBlockName(),log.getBaseEntityId());
            ...
            case PNC_REGISTRATION_BEFORE_48_hour:
                ...
                if(details.containsKey("number_of_pnc")&&!StringUtils.isEmpty(details.get("number_of_pnc"))) {
                    ...
                    HnppApplication.getIndicatorRepository().updateValue("number_of_pnc",value,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",log.getBlockName(),log.getBaseEntityId());
            ...
            case WOMEN_REFERRAL:
                if(details.containsKey("cause_of_referral_woman")&&!StringUtils.isEmpty(details.get("cause_of_referral_woman"))) {
                    ...
                    HnppApplication.getIndicatorRepository().updateValue("cause_of_referral_woman",value,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",log.getBlockName(),log.getBaseEntityId());
            ...
            case PREGNANCY_OUTCOME:
                if(details.containsKey("is_tt_completed")&&!StringUtils.isEmpty(details.get("is_tt_completed"))) {
                    ...
                    if(value.equalsIgnoreCase("yes")){
                        HnppApplication.getIndicatorRepository().updateValue(HnppConstants.INDICATOR.OUTCOME_TT,value,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",log.getBlockName(),log.getBaseEntityId());
            ...
            case CHILD_FOLLOWUP:
                ...
                if(!TextUtils.isEmpty(bfValue) && bfValue.equalsIgnoreCase("yes") && !TextUtils.isEmpty(efValue) && efValue.equalsIgnoreCase("no")){
                    HnppApplication.getIndicatorRepository().updateValue(HnppConstants.INDICATOR.FEEDING_UPTO_6_MONTH,"true",localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",log.getBlockName(),log.getBaseEntityId());
                ...
                if(details.containsKey("solid_food_month")&&!StringUtils.isEmpty(details.get("solid_food_month"))) {
                    ...
                    HnppApplication.getIndicatorRepository().updateValue("solid_food_month",value,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",log.getBlockName(),log.getBaseEntityId());
            ...
            case HnppConstants.EventType.REMOVE_MEMBER: {
                if (details.containsKey("cause_of_death") && !StringUtils.isEmpty(details.get("cause_of_death"))) {
                    ...
                    HnppApplication.getIndicatorRepository().updateValue("cause_of_death", value, localDate.getDayOfMonth() + "", localDate.getMonthOfYear() + "", localDate.getYear() + "", log.getBlockName(), log.getBaseEntityId());

                }
                if (details.containsKey("remove_reason") && !StringUtils.isEmpty(details.get("remove_reason"))) {
                    String value = details.get("remove_reason");
                    HnppApplication.getIndicatorRepository().updateValue("remove_reason", value, localDate.getDayOfMonth() + "", localDate.getMonthOfYear() + "", localDate.getYear() + "", log.getBlockName(), log.getBaseEntityId());
            ...
            case HOME_VISIT_FAMILY:
                if(details.containsKey("is_affected_member")&&!StringUtils.isEmpty(details.get("is_affected_member"))) {
                    ...
                    HnppApplication.getIndicatorRepository().updateValue("is_affected_member",value,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",log.getBlockName(),log.getBaseEntityId());

                }
                if(details.containsKey("member_count")&&!StringUtils.isEmpty(details.get("member_count"))) {
                    String value = details.get("member_count");
                    HnppApplication.getIndicatorRepository().updateValue("member_count",value,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",log.getBlockName(),log.getBaseEntityId());
            ...
            case CORONA_INDIVIDUAL:
                if(details.containsKey("corona_test_result")&&!StringUtils.isEmpty(details.get("corona_test_result"))) {
                    ...
                    HnppApplication.getIndicatorRepository().updateValue("corona_test_result",value,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",log.getBlockName(),log.getBaseEntityId());

                }
                if(details.containsKey("isolation")&&!StringUtils.isEmpty(details.get("isolation"))) {
                    String value = details.get("isolation");
                    HnppApplication.getIndicatorRepository().updateValue("isolation",value,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",log.getBlockName(),log.getBaseEntityId());
        ...
        if( HnppConstants.EVENT_TYPE.CHILD_REFERRAL.equalsIgnoreCase(encounter_type)){
            ...
            if(details.containsKey("cause_of_referral_all")&&!StringUtils.isEmpty(details.get("cause_of_referral_all"))){
                ...
                if(!TextUtils.isEmpty(cataractRefer) && cataractRefer.equalsIgnoreCase("cataract_problem")){
                    LocalDate localDate = new LocalDate(log.getVisitDate());
                    HnppApplication.getTargetRepository().updateValue(HnppConstants.EVENT_TYPE.CATARACT_SURGERY_REFER,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",log.getBlockName(),log.getBaseEntityId(),formSubmissionId);
        ...
        if(REFERREL_FOLLOWUP.equalsIgnoreCase(encounter_type)){
            ...
            if(details.containsKey("is_operation_done")&&!StringUtils.isEmpty(details.get("is_operation_done"))){
                ...
                if(!TextUtils.isEmpty(operationDone) && operationDone.equalsIgnoreCase("Yes")){
                    LocalDate localDate = new LocalDate(log.getVisitDate());
                    HnppApplication.getTargetRepository().updateValue(HnppConstants.EVENT_TYPE.CATARACT_SURGERY,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",log.getBlockName(),log.getBaseEntityId(),formSubmissionId);
    ...
    private static void processEyeTest(HashMap<String, String> details, VisitLog visit, String formSubmissionId) {
        if(details!=null){
            if(details.containsKey("exam_result") && !StringUtils.isEmpty(details.get("exam_result"))) {
                ...
                if(!TextUtils.isEmpty(known) && known.equalsIgnoreCase("presbiopia")){
                    ...
                    HnppApplication.getTargetRepository().updateValue(HnppConstants.EVENT_TYPE.MARKED_PRESBYOPIA,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",visit.getBlockName(),visit.getBaseEntityId(),formSubmissionId);
                }
            }
            if(details.containsKey("glasses_sell") && !StringUtils.isEmpty(details.get("glasses_sell"))) {
                ...
                if(!TextUtils.isEmpty(value)){
                    LocalDate localDate = new LocalDate(visit.getVisitDate());
                    HnppApplication.getIndicatorRepository().updateValue("glasses_sell", value, localDate.getDayOfMonth() + "", localDate.getMonthOfYear() + "", localDate.getYear() + "", visit.getBlockName(), visit.getBaseEntityId());
                }
            }
            if(details.containsKey("is_need_glasses") && !StringUtils.isEmpty(details.get("is_need_glasses"))) {
                ...
                if(!TextUtils.isEmpty(known) && known.equalsIgnoreCase("yes")){
                    LocalDate localDate = new LocalDate(visit.getVisitDate());
                    HnppApplication.getTargetRepository().updateValue(HnppConstants.EVENT_TYPE.PRESBYOPIA_CORRECTION,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",visit.getBlockName(),visit.getBaseEntityId(),formSubmissionId);
                    if(details.containsKey("add_to_stock") && !StringUtils.isEmpty(details.get("add_to_stock"))) {
                        String add_to_stock = details.get("add_to_stock");
                        if (!TextUtils.isEmpty(add_to_stock) && !add_to_stock.equalsIgnoreCase("0")) {
                            HnppApplication.getStockRepository().updateValue(HnppConstants.EVENT_TYPE.GLASS,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",visit.getBlockName(),visit.getBaseEntityId(),visit.getVisitDate(),formSubmissionId);
            ...
            if(details.containsKey("glasses_type") && !StringUtils.isEmpty(details.get("glasses_type"))) {
                ...
                if(!TextUtils.isEmpty(known) && known.equalsIgnoreCase("sv")){
                        ...
                        if(!TextUtils.isEmpty(power)) {
                            if(details.containsKey("add_to_stock") && !StringUtils.isEmpty(details.get("add_to_stock"))) {
                                ...
                                if (!TextUtils.isEmpty(add_to_stock) && !add_to_stock.equalsIgnoreCase("0")) {
                                    ...
                                    switch (power){
                                        case "1":
                                            HnppApplication.getStockRepository().updateValue(HnppConstants.EVENT_TYPE.SV_1,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",visit.getBlockName(),visit.getBaseEntityId(),visit.getVisitDate(),formSubmissionId);
                                            break;
                                        case "1.5":
                                            HnppApplication.getStockRepository().updateValue(HnppConstants.EVENT_TYPE.SV_1_5,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",visit.getBlockName(),visit.getBaseEntityId(),visit.getVisitDate(),formSubmissionId);
                                            break;
                                        case "2":
                                            HnppApplication.getStockRepository().updateValue(HnppConstants.EVENT_TYPE.SV_2,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",visit.getBlockName(),visit.getBaseEntityId(),visit.getVisitDate(),formSubmissionId);
                                            break;
                                        case "2.5":
                                            HnppApplication.getStockRepository().updateValue(HnppConstants.EVENT_TYPE.SV_2_5,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",visit.getBlockName(),visit.getBaseEntityId(),visit.getVisitDate(),formSubmissionId);
                                            break;
                                        case "3":
                                            HnppApplication.getStockRepository().updateValue(HnppConstants.EVENT_TYPE.SV_3,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",visit.getBlockName(),visit.getBaseEntityId(),visit.getVisitDate(),formSubmissionId);
                                            break;
                        ...
                        if(!TextUtils.isEmpty(power)) {
                            if(details.containsKey("add_to_stock") && !StringUtils.isEmpty(details.get("add_to_stock"))) {
                                ...
                                if (!TextUtils.isEmpty(add_to_stock) && !add_to_stock.equalsIgnoreCase("0")) {
                                    ...
                                    switch (power){
                                        case "1":
                                            HnppApplication.getStockRepository().updateValue(HnppConstants.EVENT_TYPE.BF_1,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",visit.getBlockName(),visit.getBaseEntityId(),visit.getVisitDate(),formSubmissionId);
                                            break;
                                        case "1.5":
                                            HnppApplication.getStockRepository().updateValue(HnppConstants.EVENT_TYPE.BF_1_5,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",visit.getBlockName(),visit.getBaseEntityId(),visit.getVisitDate(),formSubmissionId);
                                            break;
                                        case "2":
                                            HnppApplication.getStockRepository().updateValue(HnppConstants.EVENT_TYPE.BF_2,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",visit.getBlockName(),visit.getBaseEntityId(),visit.getVisitDate(),formSubmissionId);
                                            break;
                                        case "2.5":
                                            HnppApplication.getStockRepository().updateValue(HnppConstants.EVENT_TYPE.BF_2_5,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",visit.getBlockName(),visit.getBaseEntityId(),visit.getVisitDate(),formSubmissionId);
                                            break;
                                        case "3":
                                            HnppApplication.getStockRepository().updateValue(HnppConstants.EVENT_TYPE.BF_3,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",visit.getBlockName(),visit.getBaseEntityId(),visit.getVisitDate(),formSubmissionId);
                                            break;
                    ...
                    if(details.containsKey("add_to_stock") && !StringUtils.isEmpty(details.get("add_to_stock"))) {
                        ...
                        if (!TextUtils.isEmpty(add_to_stock) && !add_to_stock.equalsIgnoreCase("0")) {
                            LocalDate localDate = new LocalDate(visit.getVisitDate());
                            HnppApplication.getStockRepository().updateValue(HnppConstants.EVENT_TYPE.SUN_GLASS,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",visit.getBlockName(),visit.getBaseEntityId(),visit.getVisitDate(),formSubmissionId);
    ...
    private static void updateFamilyPlanning(VisitLog visit,HashMap<String,String>details,String formSubmissionId){
        if(details!=null){
            if(details.containsKey("familyplanning_method_known") && !StringUtils.isEmpty(details.get("familyplanning_method_known"))) {
                ...
                if(!TextUtils.isEmpty(known) && known.equalsIgnoreCase("yes")){
                    LocalDate localDate = new LocalDate(visit.getVisitDate());
                    HnppApplication.getTargetRepository().updateValue(HnppConstants.EVENT_TYPE.METHOD_USER,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",visit.getBlockName(),visit.getBaseEntityId(),formSubmissionId);
                    if(HnppDBUtils.isAdolescent(visit.getBaseEntityId())){
                        HnppApplication.getTargetRepository().updateValue(HnppConstants.EVENT_TYPE.ADO_METHOD_USER,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",visit.getBlockName(),visit.getBaseEntityId(),formSubmissionId);
    ...
    private static void updateNcdDiabeticsTarget(VisitLog visit,HashMap<String,String>details,String formSubmissionId){
        if(details.containsKey("fasting_blood_sugar") && !StringUtils.isEmpty(details.get("fasting_blood_sugar"))){
            ...
            if(!TextUtils.isEmpty(fbsValue)){
                ...
                if (nP>=7){
                    LocalDate localDate = new LocalDate(visit.getVisitDate());
                    HnppApplication.getTargetRepository().updateValue(HnppConstants.EVENT_TYPE.ESTIMATE_DIABETES,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",visit.getBlockName(),visit.getBaseEntityId(),formSubmissionId);
                    return;
                }
    ...
    private static void updateNcdBpTarget(VisitLog visit,HashMap<String,String>details,String formSubmissionId){
        LocalDate localDate = new LocalDate(visit.getVisitDate());
...
//                    if (bps>=140){
//                        HnppApplication.getTargetRepository().updateValue(HnppConstants.EVENT_TYPE.ESTIMATE_HBP,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",visit.getBlockName(),visit.getBaseEntityId());
//                    }
//                }catch (NumberFormatException e){
//
//                }
        ...
        if(details.containsKey("cause_of_ncd") && !StringUtils.isEmpty(details.get("cause_of_ncd"))){
            ...
            if(!TextUtils.isEmpty(fbsValue)){
                if (fbsValue.contains("high_blood_pressure")){
                    HnppApplication.getTargetRepository().updateValue(HnppConstants.EVENT_TYPE.ESTIMATE_HBP,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",visit.getBlockName(),visit.getBaseEntityId(),formSubmissionId);
                }
...
//            }
//        }
        //if(HnppConstants.isPALogin())HnppApplication.getTargetRepository().updateValue(NCD_BY_PA,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",visit.getBlockName(),visit.getBaseEntityId());


    }
    private static void updateNcdPackageRisk(String baseEntityId,HashMap<String,String>details){
        if(details.containsKey("fasting_blood_sugar") && !StringUtils.isEmpty(details.get("fasting_blood_sugar"))){
            String fbsValue = details.get("fasting_blood_sugar");
            if(!TextUtils.isEmpty(fbsValue)){
                float nP = Float.parseFloat(fbsValue);
    ...
    private static void updatePncRisk(String baseEntityId, HashMap<String, String> details, String encounter_type){
    ...
    private static void addSSFormToIndicator(VisitLog log,HashMap<String, String> details) {
        LocalDate localDate = new LocalDate(log.getVisitDate());
        String year = localDate.getYear()+"";
        int month = localDate.getMonthOfYear();
        String date = "01";
        if(details.containsKey("block_name")&&!StringUtils.isEmpty(details.get("block_name"))) {
            String value = details.get("block_name");
            if(!TextUtils.isEmpty(value))log.setBlockName(value);
        ...
        if(TextUtils.isEmpty(log.getBlockName())){
            String ssName = HnppDBUtils.getBlockName(log.getBaseEntityId());
            log.setBlockName(ssName);
        }
        if(details.containsKey("no_of_hh_visit") && !StringUtils.isEmpty(details.get("no_of_hh_visit"))) {
            ...
            if(!TextUtils.isEmpty(value)){
                HnppApplication.getIndicatorRepository().updateValue("no_of_hh_visit",value,date,month+"",year+"",log.getBlockName(),log.getBaseEntityId());
            }
        }
        if(details.containsKey("one_hour_after_birth") && !StringUtils.isEmpty(details.get("one_hour_after_birth"))) {
            ...
            if(!TextUtils.isEmpty(value)){
             HnppApplication.getIndicatorRepository().updateValue("one_hour_after_birth",value,date,month+"",year+"",log.getBlockName(),log.getBaseEntityId());
            }
        }
        if(details.containsKey("income_from_medicine") && !StringUtils.isEmpty(details.get("income_from_medicine"))) {
            ...
            if(!TextUtils.isEmpty(value)){
                HnppApplication.getIndicatorRepository().updateValue("income_from_medicine",value,date,month+"",year+"",log.getBlockName(),log.getBaseEntityId());
            }
        }
        if(details.containsKey("no_of_epi_present") && !StringUtils.isEmpty(details.get("no_of_epi_present"))) {
            ...
            if(!TextUtils.isEmpty(value)){
                HnppApplication.getIndicatorRepository().updateValue("no_of_epi_present",value,date,month+"",year+"",log.getBlockName(),log.getBaseEntityId());
            }
        }
        if(details.containsKey("male_patient") && !StringUtils.isEmpty(details.get("male_patient"))) {
            String value = details.get("male_patient");
            if(!TextUtils.isEmpty(value)){
                HnppApplication.getIndicatorRepository().updateValue("male_patient",value,date,month+"",year+"",log.getBlockName(),log.getBaseEntityId());
            }
        }
        if(details.containsKey("female_patient") && !StringUtils.isEmpty(details.get("female_patient"))) {
            ...
            if(!TextUtils.isEmpty(value)){
                HnppApplication.getIndicatorRepository().updateValue("female_patient",value,date,month+"",year+"",log.getBlockName(),log.getBaseEntityId());
            }
        }
        if(details.containsKey("glass_metal_count") && !StringUtils.isEmpty(details.get("glass_metal_count"))) {
            ...
            if(!TextUtils.isEmpty(value)){
                HnppApplication.getIndicatorRepository().updateValue("glass_metal_count",value,date,month+"",year+"",log.getBlockName(),log.getBaseEntityId());
            }
        }
        if(details.containsKey("glass_plastic_count") && !StringUtils.isEmpty(details.get("glass_plastic_count"))) {
            ...
            if(!TextUtils.isEmpty(value)){
                HnppApplication.getIndicatorRepository().updateValue("glass_plastic_count",value,date,month+"",year+"",log.getBlockName(),log.getBaseEntityId());
            }
        }
        if(details.containsKey("glass_sunglass_count") && !StringUtils.isEmpty(details.get("glass_sunglass_count"))) {
            ...
            if(!TextUtils.isEmpty(value)){
                HnppApplication.getIndicatorRepository().updateValue("glass_sunglass_count",value,date,month+"",year+"",log.getBlockName(),log.getBaseEntityId());
            }
        }
    }
    ...
    private static void processGMP(){
        List<Visit> v = getGMPVisitsFromEvent();
        for(Visit visit : v){
            String eventJson = visit.getJson();
            if(!StringUtils.isEmpty(eventJson)){
                try{
                    ...
                    log.setVisitJson(eventJson);
                    String ssName = HnppDBUtils.getBlockName(base_entity_id);
                    Log.v("GMP","ssname:"+ssName);
                    log.setBlockName(ssName);
                    log.setFamilyId(HnppDBUtils.getFamilyIdFromBaseEntityId(base_entity_id));
                    long rowId = HnppApplication.getHNPPInstance().getHnppVisitLogRepository().add(log);
                    Log.v("GMP","rowId:"+rowId);
    ...
    private static void updateBcg(Event baseEvent, VisitLog log){
        for (Obs o : baseEvent.getObs()) {
            if ("bcg".equalsIgnoreCase(o.getFormSubmissionField())) {
                String value = (String)o.getValue();
                LocalDate localDate = new LocalDate(log.getVisitDate());
                HnppApplication.getIndicatorRepository().updateValue("bcg",value,localDate.getDayOfMonth()+"",localDate.getMonthOfYear()+"",localDate.getYear()+"",log.getBlockName(),log.getBaseEntityId());
                break;
            }
        }
