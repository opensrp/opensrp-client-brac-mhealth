...
class BluetoothHandler {
    ...
    private final BluetoothPeripheralCallback peripheralCallback = new BluetoothPeripheralCallback() {
        ...
        public void onCharacteristicWrite(@NotNull BluetoothPeripheral peripheral, @NotNull byte[] value, @NotNull BluetoothGattCharacteristic characteristic, @NotNull GattStatus status) {
            if (status == GattStatus.SUCCESS) {
                Timber.i("SUCCESS: Writing <%s> to <%s>", BluetoothBytesParser.asHexString(value), characteristic.getUuid());
            } else {
                Timber.i("ERROR: Failed writing <%s> to <%s> (%s)", BluetoothBytesParser.asHexString(value), characteristic.getUuid(), status);
            }
        }
        ...
        public void onCharacteristicUpdate(@NotNull BluetoothPeripheral peripheral, @NotNull byte[] value, @NotNull BluetoothGattCharacteristic characteristic, @NotNull GattStatus status) {
            ...
            Log.v("BPL_CONNECT","onCharacteristicUpdate>>>"+characteristicUUID+":value:"+value);
...
//                Timber.d("%s", measurement);
 //           }
        ...
        private void writeContourClock(@NotNull BluetoothPeripheral peripheral) {
            ...
            parser.setIntValue(calendar.get(Calendar.YEAR), BluetoothBytesParser.FORMAT_UINT16);
            parser.setIntValue(calendar.get(Calendar.MONTH) + 1, BluetoothBytesParser.FORMAT_UINT8);
            parser.setIntValue(calendar.get(Calendar.DAY_OF_MONTH), BluetoothBytesParser.FORMAT_UINT8);
            parser.setIntValue(calendar.get(Calendar.HOUR_OF_DAY), BluetoothBytesParser.FORMAT_UINT8);
            parser.setIntValue(calendar.get(Calendar.MINUTE), BluetoothBytesParser.FORMAT_UINT8);
            parser.setIntValue(calendar.get(Calendar.SECOND), BluetoothBytesParser.FORMAT_UINT8);
            parser.setIntValue(offsetInMinutes + dstSavingsInMinutes, BluetoothBytesParser.FORMAT_SINT16);
